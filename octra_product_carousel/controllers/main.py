...
class octraCarousel(http.Controller):
     
        
    @http.route(['/ecommerce_product_carousel_snippets/render'], type='json', auth='public', website=True , csrf=False, cache=300)
    def render_product_carousel_slider(self,template, filter_id=False, objects_in_slide=4,style=1,limit=10, object_name=False):
    ...
    def ecommerce_product_carousel_snippets(self, template,style=1, filter_id=16, objects_in_slide=4, limit=10, object_name="product.template"):
        cr, uid, context, pool = request.cr, request.uid, request.context, request.registry
        res = self.render_product_carousel_slider(template,style=style, filter_id=filter_id, objects_in_slide=objects_in_slide, limit=limit,
