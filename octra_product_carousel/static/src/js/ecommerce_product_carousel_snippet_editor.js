odoo.define('octra_product_carousel.snippets.editor', function (require) {
'use strict';
...
options.registry.js_get_selectFilter =  options.Class.extend({
	start: function() {
	      ...
	      rpc.query({
	    	    ...
	    	    method: 'search_read',
	    	    args: [[['is_website_published','=',true],['filter_id.model_id','=','product.template']],['name','filter_id','id']],
...
options.registry.js_get_pcar_style =  options.Class.extend({
	start:function(){
      var self = this;
      setTimeout(function(){
        var ul = self.$overlay.find(".snippet-option-js_get_pcar_style > ul");
        if (self.$target.attr("data-style")) {
          var limit = self.$target.attr("data-style");
          ul.find('li[data-style="' + limit + '"]').addClass("active");
          if(limit == 2)
          {
          	self.$overlay.find('.snippet-option-js_get_type').addClass('hidden')
          	this.$target.attr("data-c_type",0)
          	this.$el.find('li[data-c_type]').removeClass('active')
          	              .filter('li[data-c_type=0]').addClass('active');
          }
          else if(limit == 3)
      	{
          	self.$overlay.find('.snippet-option-js_get_type').addClass('hidden')
      	...
      	{
          	self.$overlay.find('.snippet-option-js_get_type').removeClass('hidden')
          	this.$target.attr("data-c_type",1)
          	this.$el.find('li[data-c_type]').removeClass('active')
          	              .filter('li[data-c_type=1]').addClass('active');
      	}
         } 
        else {
    ...
    style:function(previewMode, value){
        ...
        var sty = this.$target.attr('data-style');
